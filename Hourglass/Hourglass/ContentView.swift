...
struct ContentView: View {
    ...
    @State private var selectedApps: Set<String> = [] // Stores selected apps
    @State private var timeLimits: [String: TimeLimit] = [:] // Stores time limits for selected apps
    @State private var selectedLoops: Set<String> = [] // Stores selected loop options
    @State private var selectedColor: PastelColor = .lightBlue // Track selected color, initialized to light blue
    @State private var countdownTime: TimeLimit? = nil
    @State private var remainingTime: Int = 0 // Total remaining time in seconds
    @State private var isCountingDown = false // Indicates if countdown is active
    ...
    private func toggleSelection(for platform: String) {
        if selectedApps.contains(platform) {
            ...
            timeLimits.removeValue(forKey: platform) // Remove time limit if app is deselected
        } else {
...
enum PastelColor: String, CaseIterable {
    ...
    var backgroundColor: Color {
        switch self {
        case .lightBlue: return Color.blue.opacity(0.15) // Lighter blue
        case .lightGreen: return Color.green.opacity(0.15) // Lighter green
        case .lightPink: return Color.pink.opacity(0.15) // Lighter pink
        case .lightYellow: return Color.yellow.opacity(0.15) // Lighter yellow
        }
    }
